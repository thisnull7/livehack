...
## Installation
Clone the repository: `git clone https://github.com/thisnull7/livehack.git` then `cd livehack`. Install dependencies: `pip install -r requirements.txt` (the requirements file contains requests, colorama, pyfiglet). If colorama or pyfiglet fail, the tool runs without colors in plain text. Run the tool: `python live.py` (on Termux you may need `python3 live.py`).
